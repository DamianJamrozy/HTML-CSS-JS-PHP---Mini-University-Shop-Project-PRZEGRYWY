...
<body style="overflow:hidden; color:white;">
		...
		<div id="container">
			<center><font size="4px">Użytkownicy:</font><br><br></center>
...
<th style='width:150px;'>Data Logowania</th>
<th style='width:150px;'>Typ Konta</th>
</tr>
</thead>
<tbody>
<?php
...
if (isset($_GET['page_no']) && $_GET['page_no']!="") {
	...
	$result_count = mysqli_query($database,"SELECT COUNT(*) As total_records FROM `uzytkownicy`");
...
// Pobieranie danych - userów
		$result = mysqli_query($database, "SELECT * FROM uzytkownicy LIMIT $offset, $total_records_per_page");
    ...
    <?php if($page_no < $total_no_of_pages){
		...
		<div class="left_l">
			<form method="POST" action="../../php/root.php">
				...
				<h3>Edytuj typ konta:</h3><br>
				<select class="select" id="get_val" name="Select_id_stat" required>  
						<option>Wybierz login użytkownika</option>  
						<?php 
							$i = 0;
							while($length_user!=$i){
								print_r("<option>".$logins[$i]."</option>  "); 
								$i++;
							}
						?>
					</select><br><br>
					...
					<select class="select" id="get_val" name="Select_stat" required>  
						<option>Wybierz typ konta</option>  
						<option>admin</option>  
				...
				<input type="submit" value="Edytuj" class="btn_buy" name="edit_type">
		...
		<div class="right_r">
			<form method="POST" action="../../php/root.php">
				...
				<input type="submit" class="btn_buy" value="Usuń" name="delete_user"><br><br>
			</form>
		</div>
