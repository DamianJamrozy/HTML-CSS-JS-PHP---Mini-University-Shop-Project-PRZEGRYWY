...
<body style="overflow:hidden; color:white;">
		...
		<div id="container">
			...
			<?php 
				$id_moderatora=$_POST['id_check'];
				$result = mysqli_query($database,"SELECT login FROM uzytkownicy WHERE id_uzytkownika=$id_moderatora");
				while($row = mysqli_fetch_array($result)){
					echo ("Rozmowy moderatora ".$row['login'].":");
				}
			?>
			</font><br><br></center>
...
if (isset($_GET['page_no']) && $_GET['page_no']!="") {
	...
	$i=0;
    $result = mysqli_query($database,"SELECT id_uzytkownika, imie, nazwisko FROM uzytkownicy WHERE id_uzytkownika IN ( SELECT id_uzytkownika FROM uzytkownicy_messenger WHERE id_moderatora=$id_moderatora) LIMIT $offset, $total_records_per_page");
    while($row = mysqli_fetch_array($result)){
		echo "<tr>
			  <td>".$row['id_uzytkownika']."</td>
			  <td>".$row['imie']."</td>
			  <td>".$row['nazwisko']."</td>
			  <td style='padding: 0;margin: 0;'><form id='' action='admin_messenger_last.php' method='POST' style='padding: 0; margin:0;'><input type='hidden' name='id_check' value='$row[id_uzytkownika]'><input type='submit' value='Podgląd' name='podglad' style='padding: 0;width: 100%;height: 39px; background:none;'></form></td>
		   	  </tr>";
        }
		$i++;
