...
<body style="overflow:hidden; color:white;">
		...
		<div id="container">
			<center><font size="4px">Zamówienia - Archiwum:</font><br><br></center>
...
if (isset($_GET['page_no']) && $_GET['page_no']!="") {
	...
	$result_count = mysqli_query($database,"SELECT COUNT(*) As total_records FROM `zamowienia` WHERE id_statusu=5 OR id_statusu=6");
	...
	$second_last = $total_no_of_pages - 1; // total page minus 1
		...
		// Pobieranie zamowien
	$zap2 = "SELECT * FROM zamowienia WHERE id_statusu=5 OR id_statusu=6 ORDER BY id_zamowienia LIMIT $offset, $total_records_per_page";
		...
		while($row5 = mysqli_fetch_array($sql_zam4)) {	
			echo($row5['wersja_gry']."<br><br>");
			 ...
			 echo("</td>");
			   ...
			   // Pobieranie miasta sklepu
				$zaps2 = "SELECT A.miasto_sklepu FROM sklepy A WHERE A.id_sklepu in (SELECT B.id_sklepu FROM zamowienia B WHERE B.id_zamowienia='".$row['id_zamowienia']."');";
				$sql_zams2 = mysqli_query($database, $zaps2); 

				while($rows2 = mysqli_fetch_array($sql_zams2)) {
					echo ("<td>".$rows2['miasto_sklepu']."</td>");
				}
    ...
    <?php if($page_no < $total_no_of_pages){
		...
		<br><br><br>
			<div class="left_l">
				...
				<select class="select" id="get_val" name="Select_id_stat" required>  
						...
						<?php 
							$i = 0;
							while($length_zamQ!=$i){
								print_r("<option>".$id_zamQ[$i]."</option>  "); 
								$i++;
							}
						?>
					</select><br><br>
				...
				<input type="submit" value="Edytuj" class="btn_buy" name="delete_order">
			...
			<div class="right_r">
				...
				<input type="submit" value="Usuń" class="btn_buy" name="delete_order">
				</form><br><br><br><br>
			</div>
