...
<body style="overflow:hidden; color:white;">
		...
		<div id="container">
			<center><font size="4px">Zamówienia - Dostawa:</font><br><br></center>
...
<th style='width:110px;'>Platforma</th>
<th style='width:80px;'>Ilość Kopii</th>
<th style='width:130px;'>Edycja</th>
<th style='width:110px;'>Wersja</th>
<th style='width:110px;'>Telefon</th>
<th style='width:150px;'>Koszt</th>
<th style='width:150px;'>Typ dostawy</th>
<th style='width:330px;'>Adres dostawy</th>
<th style='width:330px;'>Imie i Nazwisko</th>
<th style='width:100px;'>Data Zamówienia</th>
...
if (isset($_GET['page_no']) && $_GET['page_no']!="") {
	...
	$result_count = mysqli_query($database,"SELECT COUNT(*) As total_records FROM `zamowienia` WHERE id_dostawy=3 AND id_statusu=1 OR id_dostawy=3 AND id_statusu=2 OR id_dostawy=3 AND id_statusu=3 OR id_dostawy=3 AND id_statusu=4 OR id_dostawy=3 AND id_statusu=7 OR id_dostawy=4 AND id_statusu=1 OR id_dostawy=4 AND id_statusu=2 OR id_dostawy=4 AND id_statusu=3 OR id_dostawy=4 AND id_statusu=4 OR id_dostawy=4 AND id_statusu=7");
	...
	$second_last = $total_no_of_pages - 1; // total page minus 1
		...
		// Pobieranie zamowien
	$zap2 = "SELECT * FROM zamowienia WHERE id_dostawy=3 AND id_statusu=1 OR id_dostawy=3 AND id_statusu=2 OR id_dostawy=3 AND id_statusu=3 OR id_dostawy=3 AND id_statusu=4 OR id_dostawy=3 AND id_statusu=7 OR id_dostawy=4 AND id_statusu=1 OR id_dostawy=4 AND id_statusu=2 OR id_dostawy=4 AND id_statusu=3 OR id_dostawy=4 AND id_statusu=4 OR id_dostawy=4 AND id_statusu=7 ORDER BY id_zamowienia LIMIT $offset, $total_records_per_page";
	...
	echo("<td>");	
		...
		$zap4 = "SELECT A.wersja_gry FROM zamowienia_gry A WHERE A.id_zamowienia in (SELECT B.id_zamowienia FROM zamowienia B WHERE B.id_zamowienia='".$row['id_zamowienia']."') ORDER BY A.id_zamowienia_gry;";
		$sql_zam4 = mysqli_query($database, $zap4); 
		$z=0;
		while($row5 = mysqli_fetch_array($sql_zam4)) {	
			echo($row5['wersja_gry']."<br><br>");
		}
			 ...
			 echo("</td>");
			   ...
			   // Pobieranie miasta sklepu
				$zaps2 = "SELECT A.typ_dostawy FROM dostawy A WHERE A.id_dostawy in (SELECT B.id_dostawy FROM zamowienia B WHERE B.id_zamowienia='".$row['id_zamowienia']."');";
				$sql_zams2 = mysqli_query($database, $zaps2); 

				while($rows2 = mysqli_fetch_array($sql_zams2)) {
					echo ("<td>".$rows2['typ_dostawy']."</td>");
				}
			   ...
			   echo("
			   <td>".$row['miasto_dostawy']." ".$row['ulica_dostawy']." ".$row['nr_domu_dostawy']." / ".$row['nr_lokalu']."<br>".$row['kod_pocztowy_dostawy']." ".$row['poczta']."</td>
			...
			<form method="POST" action="../../php/root.php">
				...
				<select class="select" id="get_val" name="Select_id_stat" required>  
						...
						<?php 
							$i = 0;
							while($length_zamZ!=$i){
								print_r("<option>".$id_zamZ[$i]."</option>  "); 
								$i++;
							}
						?>
					</select><br><br>
